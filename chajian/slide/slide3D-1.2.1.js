	...
	var Slide3D = function(container, params) {
		...
		var options = {
			speed: 500, // 滑动速度，即slider自动滑动开始到结束的时间（单位ms）
						//自动循环时，设置transition-duration为0（此时不想让肉眼看出），而通过分页器点击切换时，transition-duration为speed值（此时需要过渡）
			effect: 'slide', //切换效果 flip | turn | slide | flat | fragment
			direction: 'horizontal', // 切换方向,只有slide可以切换为竖直方向'vertical',其余四种不能切换为竖直方向
			distance: 30,
		...
		s.updateContainerSize = function() {
			...
			Array.prototype.forEach.call(s.slides, function(slide, i) {  /*对每个slide3D设置宽高*/
		...
		s.addPagination = function() {
			...
			if(s.params.pagination && s.pagination) {
				...
				for(var i = 0; i < s.totalLength; i++) {
					...
					if(s.params.paginationClickable) {   /*监听圆点是否被点击*/
		...
		s.executeAnimate = function() {
			function end() {
				if(s.params.pagination) {//动画结束，更新分页器
					...
					bullets[s.activeIndex].classList.add('slide3D-pagination-bullet-active');/*给当前active的圆点添加类*/
				};
				s.restartInterval();//重置间歇调用
			};
			var animation = {  //这里只针对flip | turn| flat | fragment，因为它们需要分解图片，而slide动画在380行定义 
				turn: {   /*turn方式*/
					transitionstart: function(s) {
						s.wrapper.style.backgroundImage = 'none';
					},
					animate: function(item, fn, fnEnd, index) {//execute中调用它
						var rotation = 0;
						var data = [];
					...
					execute: function(item) {//372行执行execute
						(function(item) {
							setTimeout(function() {//超时调用，超过指定的时间后执行代码
			...
			var r = s.params.box.rows;//分成多少行
			var c = s.params.box.cols;//分成多少列
			var width = Math.ceil(s.params.width / c);//每个小格的宽度
			var height = Math.ceil(s.params.height / r);//每个小格的高度
			var length = r * c;//小格的个数
			var total = r * c;
			var cssText = '';
			for(var i = 0; i < length; i++) {
				var left = i % c * width;//取得绝对位置偏移量
				var top = Math.floor(i / c) * height;
				var flipItem = document.createElement('div');//这个div是一个小格
				if(s.params.effect == 'fragment') {
					cssText += 'opacity:0;';
				};
				cssText += 'position:absolute;left:' + left + 'px;top:' + top + 'px;';//确定位置
				cssText += 'width:' + width + 'px;height:' + height + 'px;';//设置宽高
				cssText += 'background-image:url(' + url + ');';//背景图片，每次获取一小块
				cssText += 'background-position:-' + left + 'px -' + top + 'px;';//背景图片位置
				cssText += 'background-size:' + s.params.width + 'px ' + s.params.height + 'px;';
				cssText += 'background-repeat:no-repeat;';
				flipItem.style.cssText = cssText;//写入样式
				flipItem.change = false;
				animation[s.params.effect].execute(flipItem);//执行动画，传入flipItem这个div
				s.wrapper.appendChild(flipItem);//将length个div追加到wrapper中
			};
			//3.最后调用动画结束部分函数
			animation[s.params.effect].transitionend && animation[s.params.effect].transitionend(s);
		};
		s.effects = {
			//slide动画，分为horizontal和vertical两个方向
			slide: {
				init: function() {
					if(s.params.direction == 'horizontal') {
						if(s.params.loop) {//实际就是将第一个幻灯片选项和最后一个幻灯片选项都复制一份，然后作为最后一个幻灯片选项和第一个幻灯片选项放置进wrapper3D 里
					...
					if(s.params.loop) {//要循环播放，则克隆首尾
						var firstChild = s.wrapper.firstElementChild.cloneNode(true);
						var lastChild = s.wrapper.lastElementChild.cloneNode(true);
						s.wrapper.appendChild(firstChild);//实现了第一个幻灯片选项向左切换时，切换到最后一个；最后一个幻灯片选项向右切换时，切换到第一个。
						s.wrapper.insertBefore(lastChild, s.wrapper.firstElementChild);
						s.setTransitionDuration(s.wrapper, 0);//这是因为当过渡时间（transition-duration）为0时，切换会很快，我们肉眼是很难看到切换过程的。
						s.currentIndex = 1;
						s.moveTo(s.activeIndex + 1);//moveTo()切换到某一页
					};
				},
				touchmove: function() {//当手指在屏幕上滑动时连续地触发
					if(s.params.direction == 'horizontal') {
				...
				move: function(offset) {//滑动或点击左右按钮，distance为基准值，超过它才能实现切换
					var isMove = Math.abs(offset) > s.params.distance;
					if(s.params.loop) {//自动循环
						if(offset < 0 && isMove) {
							s.currentIndex++;
			...
			event: {
				init: function() {
					if(s.params.effect != 'slide') {//针对flip | turn| flat | fragment
						s.wrapper.innerHTML = '';
						s.wrapper.style.backgroundImage = 'url(' + s.params.sources[s.activeIndex] + ')';
		...
		s.touchMoveEvent = function(event) {
			if(s.isPressed && !s.lock) {   /*获得手指滑动的距离*/
		...
		//切换到某一页
		s.moveTo = function(index) {//实现幻灯片的切换，实质是设置wrapper3D 的transform ，每次移动的距离都是一个幻灯片选项的宽度：
			if(s.params.direction == 'horizontal') {//初始值是0，所以后面的移动是负值，即往左移或往上移
				s.setTransform(s.wrapper, 'translate3d(-' + (s.params.width * index) + 'px, 0, 0)');//translate3d有加速的作用
			} else if(s.params.direction == 'vertical') {
				s.setTransform(s.wrapper, 'translate3d(0, -' + (s.params.height * index) + 'px, 0)');
			}
		};
		s.slideTo = function(index) {//点击分页器，执行切换
			if(s.lock) return;
			s.lock = true;
			s.stopInterval();
			if(s.params.effect == 'slide') {
		...
		s.slidePrev = function() {
			...
			if(s.params.effect == 'slide') {
				s.setTransitionDuration(s.wrapper, s.params.speed);
				s.effects.slide.move(40);//模拟手指滑动
			} else {
				s.currentIndex = s.activeIndex;
				s.activeIndex--;
				s.updateActiveIndex();
		...
		s.stopImageDrag = function() {
			...
			for(var i = 0; i < images.length; i++) {
				addEvent(images[i], 'dragstart', stopDrag, false);//false表示在冒泡阶段调用事件处理程序
			}
		};
		//监听过渡开始
